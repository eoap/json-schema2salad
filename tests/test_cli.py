...
class MainCommandTests(unittest.TestCase):
    def test_main_inlines_external_schema_into_single_output_document(self) -> None:
        ...
        graph_names = [item.get('name') for item in merged_document['$graph'] if 'name' in item]
    ...
    def test_main_imports_string_format_schema_when_format_references_are_used(self) -> None:
        runner = CliRunner()
    ...
    def test_main_merges_multiple_input_schemas_into_one_document(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(cli.main, ['--output', str(output), str(first), str(second)])
    ...
    def test_main_dedupes_shared_external_schema_loaded_from_multiple_entities(self) -> None:
        runner = CliRunner()
        ...
        graph_names = [item.get('name') for item in merged_document['$graph'] if 'name' in item]
        first_record = next(item for item in merged_document['$graph'] if item.get('name') == 'First')
        second_record = next(item for item in merged_document['$graph'] if item.get('name') == 'Second')
    ...
    def test_main_qualifies_external_extends_with_source_filename(self) -> None:
        ...
        derived_record = next(item for item in merged_document['$graph'] if item.get('name') == 'Derived')

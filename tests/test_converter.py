...
class ConversionDocumentationTests(unittest.TestCase):
    def test_field_doc_appends_example_when_description_is_present(self) -> None:
        ...
        root_record = next(
            type_def for type_def in document.graph if type_def.name == "Root"
        )
...
class StringFormatConversionTests(unittest.TestCase):
    ...
    def test_string_format_maps_to_eoap_salad_reference(self) -> None:
        ...
        root_record = next(
            type_def
            for type_def in document.graph
            if getattr(type_def, "name", None) == "Root"
        )
        field = root_record.fields[0]
    ...
    def test_string_format_applies_inside_arrays_and_nullable_unions(self) -> None:
        ...
        timestamps = next(
            field for field in root_record.fields if field.name == "timestamps"
        )
        callback = next(
            field for field in root_record.fields if field.name == "callback"
        )
    ...
    def test_all_eoap_string_formats_are_supported(self) -> None:
        ...
        for format_name, record_name in expected_refs.items():
            ...
            self.assertEqual(
                f"string_format:{record_name}", fields_by_name[field_name].type
            )
...
class RecordNamingTests(unittest.TestCase):
    def test_root_object_record_uses_sanitized_title(self) -> None:
        ...
        self.assertEqual([], warnings)
        self.assertTrue(
            any(type_def.name == "Order_Root_v2" for type_def in document.graph)
        )
    ...
    def test_nested_object_record_uses_title_when_present(self) -> None:
        ...
        contact_field = next(
            field for field in root_record.fields if field.name == "contact"
        )

        self.assertEqual([], warnings)
        self.assertEqual("Contact_Details_v2", contact_field.type)
        self.assertTrue(
            any(type_def.name == "Contact_Details_v2" for type_def in document.graph)
        )
    ...
    def test_nested_object_record_keeps_field_based_name_without_title(self) -> None:
        ...
        self.assertTrue(
            any(type_def.name == "Root_contact" for type_def in document.graph)
        )
    ...
    def test_definition_object_ref_uses_title_record_name(self) -> None:
        ...
        billing_field = next(
            field for field in root_record.fields if field.name == "billing"
        )
...
class DuplicateSourceRefTests(unittest.TestCase):
    def test_repeated_titled_inline_object_reuses_existing_record(self) -> None:
        ...
        geometry = next(
            field for field in root_record.fields if field.name == "geometry"
        )
        nested_geometry = next(
            field for field in root_record.fields if field.name == "nested_geometry"
        )
        graph_names = [type_def.name for type_def in document.graph]
    ...
    def test_matching_inline_title_with_different_schema_keeps_accumulator(
        self,
    ) -> None:
        schema = {
            "title": "Root",
    ...
    def test_existing_json_ref_source_reuses_name_without_accumulator(self) -> None:
        ...
        root_record = next(
            type_def for type_def in converted.document.graph if type_def.name == "Root"
        )
        thing_field = next(
            field for field in root_record.fields if field.name == "thing"
        )
        graph_names = [type_def.name for type_def in converted.document.graph]
        thing_record = next(
            type_def
            for type_def in converted.document.graph
            if type_def.name == "Thing"
        )
    ...
    def test_matching_title_from_different_json_ref_source_still_gets_accumulator(
        self,
    ) -> None:
        schema = {
            "title": "Root",
